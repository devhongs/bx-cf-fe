...
### 01. 프로젝트 기반 구축
- [x] 모노레포 구조 설정 (turborepo + pnpm workspace)
  * apps/* (pc-web, mobile-web, admin-portal) 및 packages/* 분리
  * 워크스페이스 의존성 링크 (workspace:*)
- [x] 번들러 설정 (Vite, 앱별 포트 분리)
- [x] TypeScript 설정 (tsconfig, 경로 alias)
...
- [ ] 코드 품질 도구 설정
  * Biome (lint + format 완료)
  * Husky pre-commit 훅
- [x] 공통 패키지 (@bx/shared) 구조 확정
  * entities / shared(ui·hooks·model·lib·types·constants·ajax) 레이어
  * 배럴(index.ts) export 규칙, 패키지 내부 상대경로 임포트(순환참조 방지)
### 02. 라우팅 시스템
- [x] TanStack Router 기본 설정 (앱별)
  * 라우트 그룹 전략: (auth) 로그인 / (page) 보호 라우트 / (modal) 모달
- [x] 라우트 트리 자동생성 (router-plugin, routeTree.gen 수정 금지)
- [x] 인증 가드 구현 (requireAuth)
- [x] 404 / 리다이렉션 처리
- [ ] 라우트 파라미터 필수값 체크
- [ ] modal 에서 일반 페이지 라우팅 시 modal 닫히면서 기존 페이지 보여지는 현상 제어
- [x] 앱 라우팅 시 이동된 페이지 스크롤 top 이동
### 03. 인증
- [x] 로그인 폼 UI (id / password / submit 버튼)
- [x] 로그인 API 연동
  * 비밀번호 해싱(sha256) 후 전송 정책
- [x] 토큰 저장 전략 결정 (localStorage / cookie / sessionStorage)
- [x] 로그인 실패 에러 처리
- [x] Access token 만료 → silent refresh 처리
  * 동시 요청 single-flight 처리 (refresh 1회만 호출)
- [ ] Refresh token rotation 전략
- [x] 로그아웃 처리
- [x] 로그인 상태에 따른 라우팅 제어 (PrivateRoute)
- [ ] 다중 탭 세션 동기화 (storage 이벤트)
### 04. API 연동 기반
- [x] Axios 인스턴스 설정 (baseURL, timeout, headers)
  * httpService.init() 단일 진입점 패턴
- [x] 공통 응답 규격(envelope) 정의
  * { success, code, msg, payload } — payload 자동 언래핑
- [x] 에러코드 체계 표준화 (API_ERROR_CODE)
...
- [x] Axios interceptor
  ...
  * 응답: HTTP 상태코드별 공통 처리
  ...
  - 403 → 권한 페이지
  - 500 → 공통 에러 페이지
- [x] React Query 글로벌 설정 (QueryClient, staleTime, retry)
...
- [ ] 관리자 화면
  ...
  * 사용자 관리
### 08. 상태 관리
- [x] Zustand — 클라이언트 전역 상태 (인증, 사용자 정보)
- [x] React Query — 서버 상태 (API 캐싱, 동기화)
- [x] 스토리지 추상화 (localStorage / sessionStorage 유틸)
- [x] 저장 키 상수화 (STORAGE_KEYS)
### 21. 개발 표준 & 협업 규칙
- [ ] 에디터/OS 일관성 설정
  * .editorconfig (인덴트·개행)
