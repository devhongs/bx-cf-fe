...
- Markdown의 `h2`를 상위 메뉴, `h3`를 하위 메뉴로 사용한다.
- `h1`은 문서 제목이므로 목차에서 제외한다.
- 기존 Markdown 본문 스타일과 단일 HTML 산출물 구조를 유지한다.
- 모바일용 별도 목차 UI나 페이지 분할은 추가하지 않는다.

## Generation

`marked.lexer()`로 Markdown을 토큰화하고 heading 토큰에 고유 ID를 부여한다. 본문과 목차는 같은 토큰 정보를 사용해 서로 다른 ID가 생성되지 않게 한다.
...
- `h3` 항목은 `h2`보다 들여써 계층을 표현한다.
- 항목 클릭 시 URL hash를 갱신하고 해당 heading으로 부드럽게 스크롤한다.
- 스크롤 위치에 따라 현재 문단의 목차 항목을 강조한다.
