...
vi.mock('@/shared/guards', () => ({ requireAuth: mocks.requireAuth }));
vi.mock('@/widgets/layout/footer/Footer', () => ({ Footer: () => null }));
vi.mock('@/widgets/layout/header/Header', () => ({ Header: () => null }));

import './_page';

describe('Mobile protected page loader', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  ...
  it('authenticates before bootstrapping base info with an app-specific menu scope', async () => {
    ...
    expect(mocks.createBaseInfoMenuCacheScope).toHaveBeenCalledWith('mobile', 'hongsik.yoo');
    expect(mocks.ensureBaseInfoBootstrapped).toHaveBeenCalledWith(mocks.queryClient, {
      menuCacheScope: 'mobile:hongsik.yoo',
    });
